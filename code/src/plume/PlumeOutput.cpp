  ...
  : QESNetCDFOutput(output_file)
{
  ...
  */
  ...
  ongoingAveragingTime = 0.0f;

#ifdef HAS_CUDA_SUPPORT
  advectParticlesGPU_demo_prepareEulerianBoxes(
      nBoxesX, nBoxesY, nBoxesZ,
      lBndx, lBndy, lBndz,
      boxSizeX, boxSizeY, boxSizeZ);
  advectParticlesGPU_demo_resetEulerianBoxes();
  gpuEulerPrepared = true;
#endif
}
...
#ifdef HAS_CUDA_SUPPORT
  if (gpuEulerPrepared) {
    // GPU 路径：每步只在 device 端累计
    advectParticlesGPU_demo_accumulateEulerianBoxes(timeStep);

    if (timeIn >= nextOutputTime) {
      // 真正输出时才刷回 host
      advectParticlesGPU_demo_flushEulerianBoxesToHost(pBox, conc, true);
      advectParticlesGPU_demo_flushDepositionToHost(m_plume->deposition);
