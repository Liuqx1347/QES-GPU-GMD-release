...
namespace {
struct PlumePerfStats {
  double emitParticles = 0.0;
  double initParticleValues = 0.0;
  double particleListInsert = 0.0;
  double newVectorAssign = 0.0;
  double gpuVectorAppend = 0.0;

  double gpuAdvectWrapper = 0.0;
  double syncFlagsToHost = 0.0;
  double cpuStatusScan = 0.0;
  double outputSave = 0.0;

  double syncAllForScrub = 0.0;
  double scrubParticleList = 0.0;
  double rebuildGpuVector = 0.0;
  double invalidateGpuCache = 0.0;

  double finalFlushDeposition = 0.0;
  double finalSyncAll = 0.0;
  double resetGpuCache = 0.0;

  long long steps = 0;
  long long releaseCalls = 0;
  long long releasedThisRun = 0;
  long long scrubCalls = 0;
};

static PlumePerfStats gPlumePerf;

static inline std::chrono::high_resolution_clock::time_point qesPerfNow() {
  return std::chrono::high_resolution_clock::now();
}

static inline double qesPerfSecondsSince(const std::chrono::high_resolution_clock::time_point &t0) {
  return std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - t0).count();
}

static inline void qesPerfPrintLine(const char* name, double seconds, double totalSeconds) {
  const double pct = (totalSeconds > 0.0) ? 100.0 * seconds / totalSeconds : 0.0;
  std::cout << "  " << std::setw(32) << std::left << name
            << std::setw(12) << std::right << seconds << " s"
            << "  (" << std::setw(7) << pct << " %)" << std::endl;
}

static void qesPrintPlumePerfSummary(double totalSeconds) {
  std::cout << std::fixed << std::setprecision(6);
  std::cout << "\n[QES-Plume PERF] GPU/CPU timing breakdown" << std::endl;
  std::cout << "[QES-Plume PERF] total measured advection time = " << totalSeconds << " s" << std::endl;
  std::cout << "[QES-Plume PERF] steps = " << gPlumePerf.steps
            << ", release calls = " << gPlumePerf.releaseCalls
            << ", newly released particles = " << gPlumePerf.releasedThisRun
            << ", scrub calls = " << gPlumePerf.scrubCalls << std::endl;

  const double cpuRelease = gPlumePerf.emitParticles
                          + gPlumePerf.initParticleValues
                          + gPlumePerf.particleListInsert
                          + gPlumePerf.newVectorAssign;

  const double cpuNonGpu = cpuRelease
                         + gPlumePerf.gpuVectorAppend
                         + gPlumePerf.cpuStatusScan
                         + gPlumePerf.outputSave
                         + gPlumePerf.scrubParticleList
                         + gPlumePerf.rebuildGpuVector;

  const double gpuHostSync = gPlumePerf.syncFlagsToHost
                           + gPlumePerf.syncAllForScrub
                           + gPlumePerf.finalSyncAll
                           + gPlumePerf.finalFlushDeposition
                           + gPlumePerf.invalidateGpuCache
                           + gPlumePerf.resetGpuCache;

  std::cout << "\n[QES-Plume PERF] A. GPU core / wrapper" << std::endl;
  qesPerfPrintLine("gpuAdvectWrapper", gPlumePerf.gpuAdvectWrapper, totalSeconds);

  std::cout << "\n[QES-Plume PERF] B. CPU-side parts that are not worth / not easy to GPU accelerate" << std::endl;
  qesPerfPrintLine("emitParticles", gPlumePerf.emitParticles, totalSeconds);
  qesPerfPrintLine("setParticleVals", gPlumePerf.initParticleValues, totalSeconds);
  qesPerfPrintLine("particleList.insert", gPlumePerf.particleListInsert, totalSeconds);
  qesPerfPrintLine("newParticlesVec.assign", gPlumePerf.newVectorAssign, totalSeconds);
  qesPerfPrintLine("gpuParticles vector append", gPlumePerf.gpuVectorAppend, totalSeconds);
  qesPerfPrintLine("CPU active/rogue scan", gPlumePerf.cpuStatusScan, totalSeconds);
  qesPerfPrintLine("output save / NetCDF", gPlumePerf.outputSave, totalSeconds);
  qesPerfPrintLine("scrubParticleList delete", gPlumePerf.scrubParticleList, totalSeconds);

  std::cout << "\n[QES-Plume PERF] C. GPU-host synchronization / data-transfer overhead" << std::endl;
  qesPerfPrintLine("syncFlagsToHost", gPlumePerf.syncFlagsToHost, totalSeconds);
  qesPerfPrintLine("syncAllForScrub", gPlumePerf.syncAllForScrub, totalSeconds);
  qesPerfPrintLine("finalSyncAll", gPlumePerf.finalSyncAll, totalSeconds);
  qesPerfPrintLine("finalFlushDeposition", gPlumePerf.finalFlushDeposition, totalSeconds);
  qesPerfPrintLine("invalidate/reset GPU cache", gPlumePerf.invalidateGpuCache + gPlumePerf.resetGpuCache, totalSeconds);

  std::cout << "\n[QES-Plume PERF] Summary" << std::endl;
  qesPerfPrintLine("CPU non-GPU subtotal", cpuNonGpu, totalSeconds);
  qesPerfPrintLine("GPU-host sync subtotal", gpuHostSync, totalSeconds);
  qesPerfPrintLine("non-GPU + sync subtotal", cpuNonGpu + gpuHostSync, totalSeconds);
  qesPerfPrintLine("GPU wrapper subtotal", gPlumePerf.gpuAdvectWrapper, totalSeconds);
  std::cout << std::endl;
}
}
...
#pragma omp barrier
  ...
  setBCfunctions(xBCtype, yBCtype, zBCtype);
   ...
   std::cout << "[Plume] BC types: x=" << xBCtype
            << ", y=" << yBCtype
            << ", z=" << zBCtype << std::endl;
...
void Plume::run(QEStime loopTimeEnd, WINDSGeneralData *WGD, TURBGeneralData *TGD, std::vector<QESNetCDFOutput *> outputVec)
{
  ...
  std::vector<Particle*> gpuParticles;
  gpuParticles.reserve(totalParsToRelease);



  while (simTimeCurr < loopTimeEnd) {
    std::vector<Particle*> newlyReleased;
    auto tPerf0 = qesPerfNow();
    int nParsToRelease = generateParticleList(simTime, WGD, TGD, &newlyReleased);
    // generateParticleList() already records its internal emit/init/insert timings.
    gPlumePerf.releaseCalls++;
    gPlumePerf.releasedThisRun += nParsToRelease;
    ...
    int activeCountThisStep = 0;
    int inactiveCountThisStep = 0;
    int rogueCountThisStep = 0;
    ...
    tPerf0 = qesPerfNow();
    gpuParticles.insert(gpuParticles.end(), newlyReleased.begin(), newlyReleased.end());
    gPlumePerf.gpuVectorAppend += qesPerfSecondsSince(tPerf0);

#if defined(HAS_CUDA_SUPPORT)

    tPerf0 = qesPerfNow();
    advectParticlesGPU_demo(timeRemainder, gpuParticles, boxSizeZ, WGD, TGD,
                        false, 0.0, 0.0, 0.0,
                        false,
                        1.0f,
                        CourantNum,
                        ...
                        gpuBCType_y,
                        gpuBCType_z);
    gPlumePerf.gpuAdvectWrapper += qesPerfSecondsSince(tPerf0);
    gPlumePerf.steps++;

    // 轻量 flags 同步：只为统计 active / inactive / rogue
    tPerf0 = qesPerfNow();
    advectParticlesGPU_demo_syncFlagsToHost(gpuParticles);
    gPlumePerf.syncFlagsToHost += qesPerfSecondsSince(tPerf0);


#elif defined(_OPENMP)

#pragma omp parallel for default(none) shared(WGD, TGD, gpuParticles, timeRemainder, boxSizeZ)
    for (auto k = 0u; k < gpuParticles.size(); ++k) {
      advectParticle(timeRemainder, gpuParticles[k], boxSizeZ, WGD, TGD);
    }

#else

    for (auto *parItr : gpuParticles) {
...
if (!outputVec.empty()) {
  ...
  for (auto &parItr : particleList) {
    ...
    for (auto &parItr : particleList) {
      ...
      if (!parItr->isActive) {
        ...
        inactiveCountThisStep++;
        needToScrub = true;
      } else {
        activeCountThisStep++;
      }
    }
    gPlumePerf.cpuStatusScan += qesPerfSecondsSince(tPerf0);

#if !defined(HAS_CUDA_SUPPORT)
    if (needToScrub) {
      scrubParticleList();
      activeCountThisStep = static_cast<int>(particleList.size());
    }
#endif
  ...
  gPlumePerf.outputSave += qesPerfSecondsSince(tPerf0);
}
...
#if defined(HAS_CUDA_SUPPORT)
    if (needToScrub) {
      const bool triggerScrub =
          ((simTimeIdx % 60) == 0) ||
          (inactiveCountThisStep > static_cast<int>(0.45 * gpuParticles.size()));

      if (triggerScrub) {
        tPerf0 = qesPerfNow();
        advectParticlesGPU_demo_syncAllToHost(gpuParticles);
        gPlumePerf.syncAllForScrub += qesPerfSecondsSince(tPerf0);

        scrubParticleList();

        tPerf0 = qesPerfNow();
        gpuParticles.clear();
        gpuParticles.reserve(particleList.size());
        for (auto *p : particleList) {
          gpuParticles.push_back(p);
        }
        gPlumePerf.rebuildGpuVector += qesPerfSecondsSince(tPerf0);

        tPerf0 = qesPerfNow();
        advectParticlesGPU_demo_invalidateParticleCache();
        gPlumePerf.invalidateGpuCache += qesPerfSecondsSince(tPerf0);
        activeCountThisStep = static_cast<int>(particleList.size());
      }
    }
#endif
    ...
    if (simTimeCurr >= nextUpdate || (simTimeCurr == loopTimeEnd)) {
      if (verbose) {
        ...
        std::cout << "Time = " << simTimeCurr
                  ...
                  << " s, iteration = " << simTimeIdx << "). "
                  << "Particles: Released = " << nParsReleased << " "
                  << "Active = " << activeCountThisStep << "." << std::endl;
      }
  ...
  {
    auto tPerf0 = qesPerfNow();
    advectParticlesGPU_demo_flushDepositionToHost(deposition);
    gPlumePerf.finalFlushDeposition += qesPerfSecondsSince(tPerf0);

    if (!gpuParticles.empty()) {
      tPerf0 = qesPerfNow();
      advectParticlesGPU_demo_syncAllToHost(gpuParticles);
      gPlumePerf.finalSyncAll += qesPerfSecondsSince(tPerf0);
  ...
  {
    auto tPerf0 = qesPerfNow();
    advectParticlesGPU_demo_resetCache();
    gPlumePerf.resetGpuCache += qesPerfSecondsSince(tPerf0);
  }
#endif
  ...
  qesPrintPlumePerfSummary(Elapsed.count());
}
...
int Plume::generateParticleList(float currentTime,
                                WINDSGeneralData *WGD,
                                TURBGeneralData *TGD,
                                std::vector<Particle*>* newParticlesVec)
{
  ...
  gPlumePerf.emitParticles += qesPerfSecondsSince(tPerf0);

  tPerf0 = qesPerfNow();
  setParticleVals(WGD, TGD, nextSetOfParticles);
  gPlumePerf.initParticleValues += qesPerfSecondsSince(tPerf0);
  ...
  gPlumePerf.particleListInsert += qesPerfSecondsSince(tPerf0);

  if (newParticlesVec) {
    tPerf0 = qesPerfNow();
    newParticlesVec->assign(nextSetOfParticles.begin(), nextSetOfParticles.end());
    gPlumePerf.newVectorAssign += qesPerfSecondsSince(tPerf0);
  }
...
void Plume::scrubParticleList()
{
  ...
  gPlumePerf.scrubParticleList += qesPerfSecondsSince(tPerf0);
}
                           ...
                           const std::string &zBCtype)
{
  ...
  // pointer point to the required function.
